package com.gitlab.rodtell.descontivo.components
...
fun MainTextField(value: String, onValueChange: (String) -> Unit, label: String) {
    OutlinedTextField(
        ...
        onValueChange = { newValue ->
            ...
            val formattedValue = filteredValue.replace(Regex("(^[0-9]+\\.?[0-9]*).*"), "$1")
...
@Composable
fun MainButton(
    text: String, color: Color = MaterialTheme.colorScheme.primary, onClick: () -> Unit
) {
    OutlinedButton(
...
fun Alert(
    ...
    AlertDialog(onDismissRequest = onDismissClick,
        ...
        confirmButton = {
            Button(onClick = { onConfirmClick() }) { Text(text = confirmText) }
        })
}